...
interface CloseWatcher extends EventTarget {
  addEventListener: <TKey extends keyof CloseWatcherEventMap>(
    type: TKey,
    // eslint-disable-next-line ts/no-explicit-any
    listener: (this: Document, event: CloseWatcherEventMap[TKey]) => any,
  ) => void;

  removeEventListener: <TKey extends keyof CloseWatcherEventMap>(
    type: TKey,
    // eslint-disable-next-line ts/no-explicit-any
    listener: (this: Document, event: CloseWatcherEventMap[TKey]) => any,
  ) => void;
...
export const CloseWatcher =
  ...
  ?? class CloseWatcherPolyfill extends EventTarget implements CloseWatcher {
    ...
    #keydownListenerInitialized() {
      ...
      globalThis.document.addEventListener('keydown', (it) => {
        if (!it.isTrusted) {
    ...
    requestClose() {
      ...
      if (this.#firingCancelEvent) return;
    ...
    set oncancel(handler) {
      // eslint-disable-next-line ts/no-unnecessary-condition -- idk why this was made
